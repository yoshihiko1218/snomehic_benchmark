#!/usr/bin/env python3
"""
Strict **no-MD identical-method** mismatch comparison (bhmem vs yap), MAPQ stratified.
...
def main() -> None:
    ...
    ap.add_argument("reference_fasta", help="Genomic mm10.fa (for coordinate consistency)")
    ap.add_argument(
        "bisulfite_genome_dir",
        help="Bisulfite_Genome dir with CT_conversion/ and GA_conversion/",
    )
    ap.add_argument("-o", "--output-prefix", default="nm_replicate_mapq_stratified")
    ap.add_argument("--max-rows-all", type=int, default=20_000, help="Use first N joined rows (default 20k)")
    ap.add_argument(
        "--max-rows-stratum",
        type=int,
        default=40_000,
    ...
    ct_fa = f"{args.bisulfite_genome_dir.rstrip('/')}/CT_conversion/genome_mfa.CT_conversion.fa"
    ga_fa = f"{args.bisulfite_genome_dir.rstrip('/')}/GA_conversion/genome_mfa.GA_conversion.fa"
    ...
    def process_row(rec: dict) -> dict | None:
        ...
        rb = recompute_nm_from_converted_genomes_pbat_no_md(br, fa_ct, fa_ga)
        ry = recompute_nm_from_converted_genomes_pbat_no_md(yr, fa_ct, fa_ga)
        ub = rb
        uy = ry
    ...
    def stat_block(name, seq):
        ...
        if pu:
            ...
            lines.append(
                f"mean_calc_yap_minus_bhmem\t{float(np.mean(uy - ub)):.6f}"
            )
            lines.append(
                f"median_calc_yap_minus_bhmem\t{float(np.median(uy - ub)):.6f}"
            )
            lines.append(
                f"fraction_calc_yap_lt_bhmem\t{float(np.mean(uy < ub)):.6f}"
            )
        lines.append("")
    ...
    with PdfPages(pdf_path) as pdf:
        # Page 1: NM vs calculated mismatch + MAPQ distributions (R1/R2)
        ...
        def _mapq_panel(ax, v1, v2, label, n1, n2):
            if len(v1) == 0 and len(v2) == 0:
                ax.set_title(f"{label} MAPQ\n(no data)")
                return
            bins = np.arange(-0.5, 61.5, 1.0)
            if len(v1):
                ax.hist(v1, bins=bins, density=True, alpha=0.55, color="#1a5276", label=f"R1 (n={n1})")
            if len(v2):
                ax.hist(v2, bins=bins, density=True, alpha=0.55, color="#c0392b", label=f"R2 (n={n2})")
            m1 = float(np.mean(v1)) if len(v1) else float("nan")
            m2 = float(np.mean(v2)) if len(v2) else float("nan")
            md1 = float(np.median(v1)) if len(v1) else float("nan")
            md2 = float(np.median(v2)) if len(v2) else float("nan")
            ax.set_xlabel("MAPQ")
            ax.set_ylabel("Density")
            ax.set_title(
                f"{label} MAPQ\nR1 mean/med={m1:.2f}/{md1:.2f}  R2 mean/med={m2:.2f}/{md2:.2f}"
            )
            ax.legend(fontsize=7)

        b1 = [x["mapq_bhmem"] for x in a1]
        b2 = [x["mapq_bhmem"] for x in a2]
        y1 = [x["mapq_yap"] for x in a1]
        y2 = [x["mapq_yap"] for x in a2]
        _mapq_panel(axes[0, 2], b1, b2, "Bhmem", len(a1), len(a2))
        _mapq_panel(axes[1, 2], y1, y2, "Yap", len(a1), len(a2))
        fig.suptitle(
            "NM tag vs calculated mismatch (conv_no_md_min4 both BAMs)\n"
        ...
        # Page 1b: recomputed-edit distributions (R1/R2) per BAM
        fig, axes = plt.subplots(2, 2, figsize=(10, 8))

        def _edit_panel(ax, vals, label):
            arr = np.array([v for v in vals if v >= 0], dtype=np.int64)
            if len(arr) == 0:
                ax.set_title(f"{label}\n(no data)")
                return
            hi = min(int(arr.max()), 120)
            step = max(1, (hi + 1) // 45)
            bins = np.arange(-0.5, hi + 1.5, step)
            ax.hist(arr, bins=bins, density=True, color="#2e86c1", alpha=0.85, edgecolor="black")
            ax.set_xlabel("Recomputed edit")
            ax.set_ylabel("Density")
            ax.set_title(
                f"{label}\n"
                f"n={len(arr)}  mean={float(np.mean(arr)):.2f}  median={float(np.median(arr)):.2f}"
            )

        _edit_panel(axes[0, 0], [x["replicate_bhmem"] for x in a1], "Bhmem read1")
        _edit_panel(axes[0, 1], [x["replicate_bhmem"] for x in a2], "Bhmem read2")
        _edit_panel(axes[1, 0], [x["replicate_yap"] for x in a1], "Yap read1")
        _edit_panel(axes[1, 1], [x["replicate_yap"] for x in a2], "Yap read2")
        fig.suptitle("Recomputed edit distributions (conv_no_md_min4)", fontsize=11)
        fig.tight_layout()
        pdf.savefig(fig, dpi=150)
        plt.close(fig)

        # Page 2: stratum histogram and difference
        ...
        _hist_pair(
            ...
            "bhmem calc",
            "yap calc",
            "MAPQ stratum: conv_no_md_min4",
        )
        su = np.array(ry, dtype=np.float64) - np.array(rb, dtype=np.float64)
        _hist_pair(
            axes[0, 1],
            [o["replicate_bhmem"] for o in diff_o],
            [o["replicate_yap"] for o in diff_o],
            "bhmem calc",
            "yap calc",
            "Diff locus subset: conv_no_md_min4",
        )
        axes[1, 0].hist(
            su,
        ...
        axes[1, 1].text(
            ...
            "Single metric: recompute_nm_from_converted_genomes_pbat_no_md\n"
            "No MD, same function on bhmem and yap.",
            fontsize=9,
        ...
        if len(diff_o) >= 20:
            ...
            ub = [o["replicate_bhmem"] for o in diff_o]
            uy = [o["replicate_yap"] for o in diff_o]
            _hist_pair(
                axes[0],
                ub,
                uy,
                "bhmem",
                "yap",
                f"Diff locus (n={len(diff_o)}): calculated mismatch",
            )
            ...
            axes[1].set_xlabel("Calculated mismatch (yap − bhmem)")
        ...
        if len(strat_o) >= 5:
            ...
            if s1:
                ax.scatter(
                    [o["replicate_bhmem"] for o in s1],
                    [o["replicate_yap"] for o in s1],
                    alpha=0.25,
                    s=10,
                    c="#1a5276",
                    label=f"R1 (n={len(s1)})",
            ...
            if s2:
                ax.scatter(
                    [o["replicate_bhmem"] for o in s2],
                    [o["replicate_yap"] for o in s2],
                    alpha=0.25,
                    s=10,
                    c="#c0392b",
                    label=f"R2 (n={len(s2)})",
            ...
            if strat_o:
                hi = max(
                    max(o["replicate_bhmem"] for o in strat_o),
                    max(o["replicate_yap"] for o in strat_o),
                    1,
                )
            ax.plot([0, hi], [0, hi], "k--", lw=0.8, alpha=0.4)
            ax.set_xlabel("Calculated mismatch bhmem")
            ax.set_ylabel("Calculated mismatch yap")
            ax.legend()
            ax.set_title("MAPQ stratum: identical no-MD method on both alignments")
            fig.tight_layout()
            pdf.savefig(fig, dpi=150)
            plt.close(fig)

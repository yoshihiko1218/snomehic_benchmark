...
# Run AFTER clearing stale GM outputs (codes/clear_gm_stale.sh --yes).
set -euo pipefail
...
j2=$(sbatch --parsable --dependency=afterok:${j1} codes/02.alignment.sh)
echo "02.alignment.sh   -> ${j2}  (afterok ${j1})"

j3=$(sbatch --parsable --dependency=afterok:${j2} codes/03.methy_extract.sh)
echo "03.methy_extract.sh -> ${j3}  (afterok ${j2})"

j4=$(sbatch --parsable --dependency=afterok:${j3} codes/run_qc.sh)
echo "run_qc.sh         -> ${j4}  (afterok ${j3})"
